        ...
        for (auto& [k, cv] : cp.get_column_vecs())
        {
            ...
            if (cv.get_column_num() >= 2)
            {
                for (auto& col : cv.get_columns())
                {
                    ...
                    col.set_gradient_cost_abs_diff(col.get_gradient_cost() - least_gradient_cost);
                    col.set_gradient_cost_rel_diff(col.get_gradient_cost_abs_diff() / std::max(0.00001, least_gradient_cost));
